        ...
        if (ufo.isActive) {
            ...
            if (ufoSpawnCooldown <= 0 && Random.nextFloat() < 0.005f) { // 0.5% chance per frame
                ...
                coroutineScope.launch {
                    SpaceInvadersViewModel.EventBus.emit(SpaceInvadersViewModel.UiEvent.PlayUFOSound)
                    SpaceInvadersViewModel.EventBus.emit(SpaceInvadersViewModel.UiEvent.Vibrate)
                }
            } else {
                ufoSpawnCooldown--
            }
